...
void test_v210_toAny_FPS()
{
	...
	auto pElapsedFunc = [&](std::function<void()> func) {
		...
		float el = std::chrono::duration_cast<std::chrono::milliseconds>(elapsed).count();
	...
	auto PFunc = [&](int sourceW, int sourceH, int destW, int destH)
		{
			std::cout << std::format("{}x{}-->{}x{}", sourceW, sourceH, destW, destH);
	...
	PFunc(3840, 2160, 480, 270);*/
	{
		...
		pElapsedFunc([&]() {
			v210::V210_to_YUV_4_to_1(targetUYVY8_Buffer, sourceBuffer, 1920, 1080,480,270);
			});
	}
	{
		std::cout << "1920x1080-->480x270       OLD         ";
		pElapsedFunc([&]() {
	...
	std::cout << std::endl;
	{
		std::cout << "1920x1080-->960x540 V210_to_YUV_2_to_1";
		pElapsedFunc([&]() {
			v210::V210_to_YUV_2_to_1(targetUYVY8_Buffer, sourceBuffer, 1920, 1080,960,540);
			});
	}

	{
		std::cout << "1920x1080-->960x540       OLD         ";
		pElapsedFunc([&]() {
	...
	std::cout << std::endl;
	{
		std::cout << "3840x2160-->960x540 V210_to_YUV_4_to_1";
		pElapsedFunc([&]() {
			v210::V210_to_YUV_4_to_1(targetUYVY8_Buffer, sourceBuffer, 3840, 2160, 960,540);
			});
	}
	{
		std::cout << "3840x2160-->960x540       OLD         ";
		pElapsedFunc([&]() {
			v210::Scaler_3840_2160_V210_to_YUV960x540(targetUYVY8_Buffer, sourceBuffer);
			});
...
void test_1080V210(const char* _destFolder)
{
	...
	v210::V210_to_YUV_4_to_1(targetUYVY8_Buffer, sourceBuffer, 1920, 1080, 480,270);
	{
		fpD = fopen(getDestFilePath("V210_to_YUV_4_to_1_480x270"), "wb");
